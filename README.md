...
поля ещё не выполненной обработки остаются `null`. Разбор выполняется до блокировки
...
на логический документ сохраняются. Неактивный документ загрузкой не включается.
