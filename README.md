...
и выход с атомарным отзывом. Прямые разрешения управляются через API;
...
Администратор выбирает пользователя и управляет прямыми разрешениями:

```text
GET    /api/admin/users
GET    /api/admin/documents/{id}/grants
POST   /api/admin/documents/{id}/grants             JSON: user_id
...
разрешение. Управление ролями через API пока не предоставляется. Авторство документа
